...
export class BreachProtocol {
  constructor(maxBufferSize = 8, matrixSize = 5, byteRange) {
    this.bufferKey = selectRandomFrom(config.bufferKey[selectRandomFrom(Object.keys(config.bufferKey))]);
    this.byteRange = byteRange;
    this.maxBufferSize = this.bufferKey.bufferSize;
  ...
  generateDaemonSequences() {
    console.log(selectRandomFrom(config.bufferKey[selectRandomFrom(Object.keys(config.bufferKey))]));
    this.daemonSequences.push(
      this.bufferKey.split.map((tuple) => {
